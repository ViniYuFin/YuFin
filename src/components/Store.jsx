  ...
  return (
    <div className="min-h-screen bg-interface p-3 pb-20">
      <div className="max-w-4xl mx-auto">
        {/* Header compacto */}
        <div className="bg-white rounded-xl shadow-lg p-4 mb-4 border-2" style={{ borderColor: 'rgb(238, 145, 22)' }}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center justify-between">
              <h1 className="text-2xl font-yufin text-primary">🛍️ Loja YüFin - {(user?.progress?.yuCoins ?? 0)} 💰</h1>
            </div>
            <div className="text-right text-sm">
              <div className="text-gray-600">Coleção: {Math.round((ownedItems / totalItems) * 100)}%</div>
              <div className="text-gray-600">Power-ups: {(user?.progress?.activeEffects?.length ?? 0)}</div>
            </div>
          </div>
        ...
        {/* Filtros compactos */}
        <div className="bg-white rounded-xl shadow-lg p-3 mb-6 border-2" style={{ borderColor: 'rgb(238, 145, 22)' }}>
            ...
            <div className="flex flex-wrap gap-4">
              {categories.map(category => (
                <button
                  ...
                  style={{ 
                    backgroundColor: selectedCategory === category.id ? 'rgb(238, 145, 22)' : 'transparent'
                  }}
                >
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 border-2" style={{ borderColor: 'rgb(238, 145, 22)' }}>
          ...
          {filteredItems.length === 0 ? (
            <div className="text-center py-6">
              ...
              <p className="text-gray-600 text-sm">Nenhum item encontrado com esses filtros.</p>
            </div>
          ) : (
