...
app.get('/users/:userId/grade-progress', async (req, res) => {
  try {
    ...
    lessons.forEach(lesson => {
      ...
      const canAccess = canAccessLesson(student, lesson, lessons);
    ...
    const currentGradeCompletedLessons = lessons.filter(lesson => 
...
function canAccessLesson(student, lesson, allLessons) {
  const completedLessons = student.progress.completedLessons || [];
